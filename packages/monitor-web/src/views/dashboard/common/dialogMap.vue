      ...
      <div class="node-map-wrap">
        ...
        <div v-if="isGps === 1 && mapImg" class="node-map-container">
          ...
          >
            ...
            <div
              v-show="
                point['sensorValues'] && point['sensorValues'].length === 0
              "
              class="no-data"
            >
              暂无传感器
...
export default {
  ...
  methods: {
    /**
     * name, gateId,isGps, map(室内的时候才有的室内地图)
     */
    ...
    gateSwitch(val) {
      ...
      this.timers = null;
      this.mapPotionInfo = [];
      this.gateName = val.name;
      this.gateId = val.gateId;
      this.isGps = 0;
      if (val.isGps === 1) {
        this.mapImg = val.map ? `/base/org/1/file?file=${val.map}` : "";
        this.$nextTick(() => {
          this.isGps = val.isGps;
          this.$nextTick(() => {
