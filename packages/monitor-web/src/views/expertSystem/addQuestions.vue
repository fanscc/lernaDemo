<template>
  <div class="chart_conten">
    <div class="chart_conten_search">
      <el-form ref="form" :inline="true" label-width="110px">
        ...
        <el-button
          ...
          >搜索</el-button
        >
        <el-button type="primary" class="ml_20" @click="add">新增</el-button>
      </el-form>
    </div>
    ...
    <dialogAddQuestions ref="dialogAddQuestionsDom" @init="init" />
  </div>
</template>

<script>
import { getMyTopic, setPublicTopicId } from "@/api/expertSystem";
import dialogAddQuestions from "./common/dialogAddQuestions";
export default {
  name: "NeedMyAnswer",
  components: {
    dialogAddQuestions
  ...
  methods: {
    init() {
      ...
      getMyTopic(params).then(res => {
        this.tableData = res.result;
      });
    },
    add() {
      this.$refs.dialogAddQuestionsDom.open();
    },
    goAnswer(id) {
      setPublicTopicId(id, { state: 2 }).then(() => {});
      this.$router.push({
        path: "/expertSystem_hidden/answerDetails",
        query: { id: id, role: 1, statu: 1 }
      });
    }
  }
};
